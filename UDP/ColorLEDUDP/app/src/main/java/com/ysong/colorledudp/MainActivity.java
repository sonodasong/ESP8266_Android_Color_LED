...
public class MainActivity extends AppCompatActivity {
	...
	class routerLoginThread implements Runnable {
		...
		public void run() {
			try {
				byte txBuf[] = ("N" + rName).getBytes();
				DatagramPacket txPkt = new DatagramPacket(txBuf, txBuf.length, InetAddress.getByName(ip), 5678);
				s.send(txPkt);
				txBuf = ("P" + rPassword).getBytes();
				txPkt = new DatagramPacket(txBuf, txBuf.length, InetAddress.getByName(ip), 5678);
				s.send(txPkt);
